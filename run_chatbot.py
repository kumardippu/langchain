...
"""
    ...
    # Options table
    options_table = Table(
        title="[bold cyan]Available Chatbots[/bold cyan]", box=box.ROUNDED
    )
    ...
    features_table = Table(
        title="[bold yellow]Feature Comparison[/bold yellow]", box=box.ROUNDED
    )
...
def run_chatbot(choice: int):
    ...
    if choice == 1:
        ...
        try:
            ...
            console.print(
                "[yellow]💡 Make sure you have all required dependencies installed[/yellow]"
            )
...
def main():
    ...
    try:
        choice = IntPrompt.ask(
            "\n[cyan]Select chatbot (1-3)[/cyan]", choices=["1", "2", "3"]
        )
