...
class SwarmMember:
    ...
    def __init__(self, name: str, role: str, specialty: str, reasoning_style: str, 
                 temperature: float = 0.7, color: str = "white"):
        self.name = name
        self.role = role
...
Remember: You're part of a revolutionary AI swarm working together to solve complex problems!"""
    ...
    def contribute(self, problem: str, other_contributions: List[Dict] = None) -> str:
        ...
        messages = [
            ...
            self.HumanMessage(content=f"PROBLEM TO SOLVE: {problem}\n\nProvide your specialist perspective and contribution.")
        ]
    ...
    def debate_response(self, problem: str, opposing_view: str, opposing_member: str) -> str:
        """Respond to an opposing viewpoint in debate format"""
        
        messages = [
            self.SystemMessage(content=self.create_system_prompt()),
            self.HumanMessage(content=f"""DEBATE SCENARIO:
...
Be constructive and focus on improving the solution through healthy debate.""")
    ...
    def synthesize_consensus(self, problem: str, all_contributions: List[Dict]) -> str:
        ...
        messages = [
            self.SystemMessage(content=self.create_system_prompt()),
            self.HumanMessage(content=f"""CONSENSUS SYNTHESIS:
Original Problem: {problem}
...
class SwarmIntelligence:
    ...
    def display_swarm_overview(self):
        ...
        table = Table(title="🐝 AI Swarm Members", show_header=True, header_style="bold magenta")
        ...
        for member in self.swarm_members:
            table.add_row(
                ...
                member.role,
                member.specialty,
                member.reasoning_style[:40] + "..." if len(member.reasoning_style) > 40 else member.reasoning_style,
    ...
    def collect_parallel_contributions(self, problem: str) -> List[Dict]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(self.swarm_members)) as executor:
            futures = [executor.submit(get_contribution, member) for member in self.swarm_members]
        ...
        # Update stats
        self.session_stats['total_contributions'] += len([c for c in contributions if c['response']])
    ...
    def display_contributions(self, contributions: List[Dict]):
        ...
        for contrib in contributions:
            if contrib['error']:
                console.print(f"❌ [red]{contrib['name']} encountered an error: {contrib['error']}[/red]")
                continue
            
            # Create member header
            header_text = Text()
            header_text.append(f"🧠 ", style=contrib['member'].color)
            header_text.append(f"{contrib['name']}", style=f"bold {contrib['member'].color}")
    ...
    def conduct_debate_round(self, problem: str, contributions: List[Dict]) -> List[Dict]:
        ...
        for debate in debate_responses:
            ...
            header_text.append(f"{debate['name']}", style=f"bold {debate['member'].color}")
    ...
    def synthesize_consensus(self, problem: str, all_responses: List[Dict]) -> str:
        ...
        with Progress(
            ...
            progress.add_task(description="🧠 Building collective consensus...", total=None)
        ...
        for contrib in consensus_contributions:
            ...
            header_text.append(f"{contrib['name']} Consensus", style=f"bold {contrib['member'].color}")
...
The swarm has spoken! 🐝"""
    ...
    def display_session_stats(self):
        ...
        stats_text = Text()
        stats_text.append(f"📊 Swarm Intelligence Session\n\n", style="bold bright_cyan")
        stats_text.append(f"🕒 Duration: {str(duration).split('.')[0]}\n", style="bright_green")
        stats_text.append(f"🎯 Problems Solved: {self.session_stats['problems_solved']}\n", style="bright_yellow")
        stats_text.append(f"🧠 Total Contributions: {self.session_stats['total_contributions']}\n", style="bright_blue")
        stats_text.append(f"🔥 Debates Conducted: {self.session_stats['debates_conducted']}\n", style="bright_red")
        stats_text.append(f"🎯 Consensus Reached: {self.session_stats['consensus_reached']}\n", style="bright_green")
        stats_text.append(f"🐝 Swarm Members: {len(self.swarm_members)}", style="bright_magenta")
        
        console.print(Panel(stats_text, title="📈 Swarm Performance", border_style="bright_cyan"))
    ...
    def main_loop(self):
        ...
        try:
            while True:
                console.print()
                problem = Prompt.ask("🎯 [bold bright_cyan]What problem should the swarm solve?[/bold bright_cyan]")
