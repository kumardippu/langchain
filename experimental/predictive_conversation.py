...
class ConversationPatternAnalyzer:
    ...
    def analyze_question_pattern(
        self, current_question: str, previous_context: List[str]
    ) -> Dict:
        """Analyze patterns in question asking"""

        analysis = {
            "question_type": self._classify_question_type(current_question),
            "topic_depth": self._calculate_topic_depth(
    ...
    def _classify_question_type(self, question: str) -> str:
        ...
        if any(
            word in question_lower for word in ["how", "explain", "what is", "what are"]
        ):
            return "explanatory"
    ...
    def _detect_curiosity_indicators(self, question: str) -> List[str]:
        ...
        if any(
            ...
            for word in ["more", "also", "additionally", "further"]
        ):
            indicators.append("depth_seeking")

        if any(
            word in question_lower for word in ["other", "different", "alternative"]
        ):
            indicators.append("breadth_seeking")

        if any(
            word in question_lower for word in ["specifically", "exactly", "precisely"]
        ):
    ...
    def _update_curiosity_profile(self, analysis: Dict):
        ...
        for key in self.user_curiosity_profile:
            self.user_curiosity_profile[key] = min(
                1.0, self.user_curiosity_profile[key]
            )
...
class PredictiveEngine:
    ...
    def predict_next_questions(
        self, current_question: str, current_response: str, context: List[str]
    ) -> List[Dict]:
        """Predict likely next questions"""

        analysis = self.pattern_analyzer.analyze_question_pattern(
...
Return only the JSON, no other text."""
        ...
        try:
            messages = [
                ...
                self.HumanMessage(
                    content="Predict the next questions based on the conversation analysis."
                ),
            ]
...
Keep the response under 200 words but pack it with value."""
            ...
            try:
                ...
                prepared_responses[question.lower()] = (
                    f"I'd be happy to explain that! (Prepared response had an error: {e})"
                )
...
class PredictiveConversation:
    ...
    def display_predictions(self, predictions: List[Dict]):
        ...
        console.print()
        console.print(
            "🔮 [bold bright_magenta]My Predictions for Your Next Questions:[/bold bright_magenta]"
        )
        ...
        for i, pred in enumerate(predictions[:3], 1):
            confidence_bar = "█" * int(pred["probability"] * 10) + "░" * (
                10 - int(pred["probability"] * 10)
            )
    ...
    def check_prediction_accuracy(self, user_question: str) -> bool:
        ...
        for prediction in self.predictions:
            ...
            shared_words = set(user_question_lower.split()) & set(
                pred_question_lower.split()
            )
    ...
    def get_response(self, user_question: str) -> str:
        ...
        for prep_question, prep_response in self.prepared_responses.items():
            ...
            if len(shared_words) >= 3:
                console.print(
                    "⚡ [bright_green]Using pre-prepared response![/bright_green]"
                )
...
Provide a helpful, engaging response that matches the user's demonstrated curiosity patterns and communication style."""
    ...
    def display_session_stats(self):
        ...
        accuracy = (
            self.session_stats["predictions_hit"]
            / max(self.session_stats["predictions_made"], 1)
        ) * 100

        stats_text = Text()
        stats_text.append(
            f"🔮 Predictive Conversation Session\n\n", style="bold bright_cyan"
        )
        stats_text.append(
            f"🕒 Duration: {str(duration).split('.')[0]}\n", style="bright_green"
        )
        stats_text.append(
            f"💬 Conversations: {self.session_stats['conversations']}\n",
            style="bright_yellow",
        )
        stats_text.append(
            f"🎯 Predictions Made: {self.session_stats['predictions_made']}\n",
            style="bright_blue",
        )
        stats_text.append(
            f"✅ Predictions Hit: {self.session_stats['predictions_hit']}\n",
            style="bright_green",
        )
        stats_text.append(
        ...
        stats_text.append(
            f"🧠 Learning: Continuously improving predictions", style="bright_cyan"
        )
        ...
        console.print(
            Panel(
                Text.assemble(
                    ...
                    ("/stats", "yellow"),
                    (" - show prediction accuracy, ", "dim"),
                    ("/profile", "yellow"),
                    (" - show curiosity profile, ", "dim"),
                    ("/quit", "yellow"),
                    (" - exit", "dim"),
                    ("\n\nCreated by ", "dim"),
        ...
        try:
            while True:
                ...
                user_question = Prompt.ask(
                    "🗣️  [bold bright_cyan]Ask me anything[/bold bright_cyan]"
                )
                ...
                if user_question.lower() in ["/profile", "profile"]:
                    ...
                    for aspect, level in profile.items():
                        ...
                        profile_table.add_row(
                            aspect.replace("_", " ").title(), f"{level:.2f}", bar
                        )
                ...
                if was_predicted:
                    console.print(
                        "🎯 [bright_green]I predicted this question![/bright_green]"
                    )
                ...
                with Progress(
                    ...
                    if was_predicted:
                        progress.add_task(
                            description="⚡ Retrieving pre-prepared response...",
                            total=None,
                        )
                    else:
                        progress.add_task(
                            description="🤔 Generating response and making predictions...",
                            total=None,
                        )
                ...
                # Display response
                console.print(
                    Panel(response, title="🤖 Response", border_style="bright_green")
                )
                ...
                with Progress(
                    ...
                    progress.add_task(
                        description="🔮 Predicting your next questions...", total=None
                    )
                    ...
                    self.prepared_responses = self.predictive_engine.prepare_responses(
                        self.predictions
                    )
                ...
                if self.session_stats["conversations"] % 3 == 0:
                    ...
                    console.print()
                    console.print(
                        f"🧠 [bright_magenta]Learning from our conversation... Current accuracy: {accuracy:.1f}%[/bright_magenta]"
                    )
            ...
            console.print(
                Panel(
                    Text.assemble(
                        ...
                        ("I made ", "bright_white"),
                        (
                            f"{self.session_stats['predictions_made']}",
                            "bold bright_yellow",
                        ),
                        (" predictions with ", "bright_white"),
                        (
                            f"{(self.session_stats['predictions_hit'] / max(self.session_stats['predictions_made'], 1)) * 100:.1f}%",
                            "bold bright_green",
                        ),
                        (" accuracy!\n", "bright_white"),
