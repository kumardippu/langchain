...
class AIPersona:
    ...
    def setup_model(self):
        ...
        try:
            ...
            self.model = ChatGoogleGenerativeAI(
                model="gemini-1.5-flash", temperature=self.traits.get("creativity", 0.7)
            )
    ...
    def _create_thinking_style(self) -> str:
        ...
        return (
            ", ".join(style_elements) if style_elements else "balanced and thoughtful"
        )
...
class MultiPersonaChat:
    ...
    def display_personas_table(self):
        ...
        for i, (key, persona) in enumerate(self.personas.items(), 1):
            # Get top 2 traits
            top_traits = sorted(
                persona.traits.items(), key=lambda x: x[1], reverse=True
            )[:2]
            traits_str = ", ".join(
    ...
    def select_personas(self):
        ...
        console.print(
            "🎯 [bold bright_cyan]Select personas to chat with:[/bold bright_cyan]"
        )
        console.print(
            "💡 [dim]Enter numbers separated by commas (e.g., 1,3,5) or 'all' for everyone[/dim]"
        )
        ...
        if choice.lower() == "all":
            ...
            try:
                ...
                self.active_personas = [
                    persona_list[i - 1] for i in indices if 1 <= i <= len(persona_list)
                ]
            except (ValueError, IndexError):
                console.print(
                    "❌ [red]Invalid selection. Using default personas.[/red]"
                )
                persona_list = list(self.personas.values())
                self.active_personas = [
                    persona_list[0],
                    persona_list[1],
                    persona_list[3],
                ]  # Default: analyst, dreamer, friend
        ...
        # Update stats
        self.session_stats["personas_used"].update(
            [p.name for p in self.active_personas]
        )

        console.print()
        active_names = [f"{p.emoji} {p.name}" for p in self.active_personas]
        console.print(
            f"✅ [bright_green]Active personas:[/bright_green] {', '.join(active_names)}"
        )
    ...
    def get_responses_parallel(self, user_input: str) -> List[tuple]:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=len(self.active_personas)
        ) as executor:
            futures = [
                executor.submit(get_persona_response, persona)
    ...
    def display_responses(self, responses: List[tuple]):
        ...
        for persona, response, error in responses:
            if error:
                console.print(
                    f"❌ [red]{persona.name} encountered an error: {error}[/red]"
                )
                continue
    ...
    def display_session_stats(self):
        ...
        stats_text.append(
            f"💬 Exchanges: {self.session_stats['total_exchanges']}\n",
            style="bright_yellow",
        )
        stats_text.append(
            f"👥 Personas Used: {len(self.session_stats['personas_used'])}\n",
            style="bright_magenta",
        )
        stats_text.append(
            f"🧠 Total Responses: {self.session_stats['total_exchanges'] * len(self.active_personas)}",
            style="bright_blue",
        )

        console.print(
            Panel(
                stats_text, title="📈 Multi-Persona Session", border_style="bright_cyan"
            )
        )
        ...
        try:
            while True:
                ...
                user_input = Prompt.ask(
                    "🗣️  [bold bright_cyan]Ask all personas[/bold bright_cyan]"
                )
                ...
                if self.session_stats["total_exchanges"] % 3 == 0:
                    console.print()
                    console.print(
                        "💡 [dim]Notice how each persona approaches your question differently![/dim]"
                    )
            ...
            console.print(
                Panel(
                    Text.assemble(
                        ...
                        (
                            f"{len(self.session_stats['personas_used'])}",
                            "bold bright_yellow",
                        ),
