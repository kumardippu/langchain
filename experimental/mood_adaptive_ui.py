...
Remember: You're not just answering questions - you're providing emotional support through adaptive interaction!"""
    ...
    def display_mood_status(self, console: Console, emotions: Dict[str, float]):
        ...
        mood_table = Table(title=f"{theme['emoji']} Emotional State Analysis", show_header=True)
        ...
        for emotion, level in sorted(emotions.items(), key=lambda x: x[1], reverse=True):
            if level > 0:
        ...
        # Theme info
        theme_info = Text()
        theme_info.append(f"🎭 Current Theme: {self.current_theme.title()}\n", style="primary")
        theme_info.append(f"🎨 Style: {theme['style_desc']}\n", style="secondary") 
        theme_info.append(f"🔄 Adaptations: {self.adaptation_count}\n", style="accent")
        theme_info.append(f"📊 Mood History: {len(self.mood_history)} entries", style="text")
        
        console.print(mood_table)
        console.print()
        console.print(Panel(theme_info, title="🎭 UI Adaptation Status", border_style="border"))
    ...
    def main_loop(self):
        ...
        console.print(Panel(
            Text.assemble(
                ...
                ("\n\n💡 Start chatting and watch the interface transform based on your mood!", "dim")
            ),
        ...
        try:
            while True:
                ...
                console.print()
                user_input = Prompt.ask("💭 [text]Share your thoughts[/text]", console=console)
                ...
                if session_stats['interactions'] % 5 == 0:
                    console.print()
                    console.print(f"[accent]✨ UI has adapted {session_stats['mood_changes']} times to match your emotions![/accent]")
